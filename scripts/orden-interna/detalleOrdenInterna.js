    ...
    function cargarProductosDetalle(id_detalle_parte) {
        ...
        materiales.forEach(element => {
            ...
            <tr data-id-producto="${element.producto?.pro_codigo ?? ''}" data-id-detalle="${element.odm_id}" class="table-primary ${claseAnimacionRow}">
                ...
                <td>
                    <button class="btn btn-sm btn-outline-primary btn-historial-material position-relative" title="Historial" ${element.cantidad_historial_materiales_count > 0 ? '' : 'disabled'}>
